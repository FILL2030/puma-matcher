...
class ViewStage(override val input: List[String], override val output: String, val rowNumber : Int = 20) extends Stage(input, output) {

  override def run(config: PipelineConfig, input: List[(DataFrameType, DataFrame)]): Option[(DataFrameType, DataFrame)] = {

    input.foreach(row => {
      Logger.info(this.name.stringValue, config.name, s"Show dataFrame of type : ${row._1.stringValue}")

      input.last._2.show(rowNumber)
    })
